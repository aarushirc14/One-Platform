...
function FunnelColumn({ column }: { column: FunnelColumn }) {
  return (
    <div
      className={cn(
        'flex flex-col gap-2 rounded-lg bg-white p-2.5',
    ...
    >
      ...
      {column.segments.map((seg) => (
        <div key={seg.title} className="rounded-lg border border-neutral-200 bg-white p-2.5 shadow-sm">
          <div className="flex items-start justify-between gap-2">
            ...
            <button
              ...
              aria-label={`${seg.title}: ${INFO_SALES_IMPACT}`}
            >
              <IconInfo className="h-3.5 w-3.5" />
            </button>
          </div>
          <p className="mt-1.5 text-xl font-bold tracking-tight text-neutral-900">{seg.primaryLine}</p>
          <p className="mt-0.5 text-xs text-neutral-600">{seg.secondaryLine}</p>
